...
func (uq *UserQuery) QueryPages() *PageQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.PagesTable, user.PagesColumn),
		)
...
func (uq *UserQuery) WithPages(opts ...func(*PageQuery)) *UserQuery {
	query := (&PageClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPages; query != nil {
...
func (uq *UserQuery) loadPages(ctx context.Context, query *PageQuery, nodes []*User, init func(*User), assign func(*User, *Page)) error {
	...
	query.Where(predicate.Page(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PagesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_pages
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_pages" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_pages" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
