...
// PageEdge is the edge representation of Page.
type PageEdge struct {
	Node   *Page  `json:"node"`
	Cursor Cursor `json:"cursor"`
}

// PageConnection is the connection containing edges to Page.
...
func (c *PageConnection) build(nodes []*Page, pager *pagePager, after *Cursor, first *int, before *Cursor, last *int) {
	...
	var nodeAt func(int) *Page
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *Page {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *Page {
	...
	c.Edges = make([]*PageEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &PageEdge{
			Node:   node,
...
// PagePaginateOption enables pagination customization.
type PagePaginateOption func(*pagePager) error

// WithPageOrder configures pagination ordering.
func WithPageOrder(order *PageOrder) PagePaginateOption {
	if order == nil {
		order = DefaultPageOrder
	}
	o := *order
	return func(pager *pagePager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultPageOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithPageFilter configures pagination filter.
func WithPageFilter(filter func(*PageQuery) (*PageQuery, error)) PagePaginateOption {
	return func(pager *pagePager) error {
		if filter == nil {
			return errors.New("PageQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type pagePager struct {
	reverse bool
	order   *PageOrder
	filter  func(*PageQuery) (*PageQuery, error)
}

func newPagePager(opts []PagePaginateOption, reverse bool) (*pagePager, error) {
	pager := &pagePager{reverse: reverse}
	...
	if pager.order == nil {
		pager.order = DefaultPageOrder
	}
	return pager, nil
}

func (p *pagePager) applyFilter(query *PageQuery) (*PageQuery, error) {
	if p.filter != nil {
...
func (p *pagePager) toCursor(pa *Page) Cursor {
	return p.order.Field.toCursor(pa)
}

func (p *pagePager) applyCursors(query *PageQuery, after, before *Cursor) (*PageQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultPageOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *pagePager) applyOrder(query *PageQuery) *PageQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultPageOrder.Field {
		query = query.Order(DefaultPageOrder.Field.toTerm(direction.OrderTermOption()))
	}
...
func (p *pagePager) orderExpr(query *PageQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		...
		if p.order.Field != DefaultPageOrder.Field {
			b.Comma().Ident(DefaultPageOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to Page.
func (pa *PageQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...PagePaginateOption,
) (*PageConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newPagePager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	...
	conn := &PageConnection{Edges: []*PageEdge{}}
...
type PageOrderField struct {
	...
	Value    func(*Page) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) page.OrderOption
	toCursor func(*Page) Cursor
}

// PageOrder defines the ordering of Page.
type PageOrder struct {
	Direction OrderDirection  `json:"direction"`
	Field     *PageOrderField `json:"field"`
}

// DefaultPageOrder is the default ordering of Page.
...
// ToEdge converts Page into PageEdge.
func (pa *Page) ToEdge(order *PageOrder) *PageEdge {
	if order == nil {
		order = DefaultPageOrder
	}
	return &PageEdge{
